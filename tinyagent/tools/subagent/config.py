...
class SubagentConfig:
    ...
    def from_parent_agent(
        ...
        **overrides
        ...
        # Handle callbacks with inheritance control
        if hasattr(parent_agent, 'callbacks') and parent_agent.callbacks:
            callbacks = list(parent_agent.callbacks)  # Copy the list
            inherited_params['callbacks'] = callbacks
        ...
        # Apply overrides
        inherited_params.update(overrides)
        
        # Create and return new config
        return cls(**inherited_params)
    ...
    def to_agent_kwargs(self, exclude_subagent_params: bool = True) -> Dict[str, Any]:
        ...
        subagent_only_params = {
            'max_turns', 'timeout', 'inherit_parent_hooks', 'working_directory', 'callbacks'
        }
